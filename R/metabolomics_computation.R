...
#' @param ref (Character) Enter the name of the reference sample or the reference feature.
...
#'  \item \code{"CV"} performs 10-fold cross validation.
#'  }
          ...
          choice = "Q2", data = "all")
{
  ...
  plsda.reg <- pls::plsr(cls ~ datmat, method = "oscorespls",
                         ncomp = compNum, validation = ifelse(methodName == "LOOCV",
                                                              "LOO", "CV"))
...
met.test_normalization <- function(mSetObj,
  ...
  for(i in 1:length(names(anova_vip1_list))){
    if(!is.null(anova_vip1_list[[i]][[1]]) && !is.null(anova_vip1_list[[i]][[2]])){
      mSet_list$plot_venn_anova_vs_vip1[[i]] <- ggVennDiagram::ggVennDiagram(anova_vip1_list[[i]], color = "black", lwd = 0.8, lty = 1,label_alpha = 0, label_size = 4.5, set_size = 5.5) +
  ...
  mSet_list <- c(lapply(1:length(list_names), function(x) suppressWarnings(met.plot_PLSImpScatter(mSetObj = mSet_list[[x]], imgName = paste0("Plots/PLSDA/PLS_Scatter_coef.mean_vs_VIP1_", names(mSet_list)[x]),
  ...
  for(i in levels(mSet_list[[1]][["dataSet"]][["cls"]])){
    names(mSet_list) <- c(list_names, names(mSet_list[(length(list_names)+1):length(mSet_list)]))
    mSet_list <- c(lapply(1:length(list_names), function(x) suppressWarnings(met.plot_PLSImpScatter(mSetObj = mSet_list[[x]], imgName = paste0("Plots/PLSDA/PLS_Scatter_coef-", i, "_vs_VIP1_", names(mSet_list)[x]),
