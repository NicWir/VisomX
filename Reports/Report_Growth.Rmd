...
knit_table <- function(df, ...){
  if (is_html_output()) {
    df %>%
     ...
     colnames(df) <- colnames(df) %>% str_replace_all(.,"<br>", "\n") %>%  str_replace_all(., "_", "\\\\char`_") %>% str_replace_all(., "µ~max~", "$\\\\mu_{max}$") %>% str_replace_all(., "λ", "$\\\\lambda$") %>% str_replace_all(., "t~start~", "$t_{start}$") %>% str_replace_all(., "t~end~", "$t_{end}$") %>% str_replace_all(., "R\\^2\\^", "$R^2$") %>% str_replace_all(., "α", "$\\\\alpha$") %>% str_replace_all(., "t~shift~", "$t_{shift}$") %>% str_replace_all(., "ν", "$\\\\nu$")
     df[,1] <- str_replace_all(df[,1], "_", "\\char`_")
     df %>% kable("latex", align = "c", booktabs = T, escape = F, linesep = "", ...) %>% kable_styling(latex_options = "hold_position", full_width = F, position = "center")
   }
}
```
## Summary
...
if(any(c("a", "l") %in% control$fit.opt)){
  ...
  knit_table(table_linear, caption = "Exponential Fit")
}
```
...
if(any(c("a", "b", "m") %in% control$fit.opt)){
    table_model <- data.frame("Sample" = paste(res.table.gc$TestId, res.table.gc$AddId, res.table.gc$concentration, sep = "|"),
                             ...
                             stringsAsFactors = F, check.names = F)
    if ( any(res.table.gc$used.model == "richards") ){
    ...
    if ( any(res.table.gc$used.model == "gompertz.exp") ){
...
if(any(c("a", "b", "s") %in% control$fit.opt)){
    table_spline <- data.frame("Sample" = paste(res.table.gc$TestId, res.table.gc$AddId, res.table.gc$concentration, sep = "|"),
                             "µ~max~" = round(as.numeric(res.table.gc$mu.spline), 3),
                             "λ" = round(as.numeric(res.table.gc$lambda.spline), 3),
                             "A" = round(as.numeric(res.table.gc$A.spline), 3),
                             "smooth.fac" = res.table.gc$smooth.spline,
                             stringsAsFactors = F, check.names = F)
    if ( grofit$control$nboot.dr > 1 ){
      table_spline <- cbind(table_spline, data.frame("µ~max~.boot" = paste(round(as.numeric(res.table.gc$mu.bt), 3),"\u00B1", round(as.numeric(res.table.gc$stdmu.bt),3)), 
                                                   "λ.boot" = paste(round(as.numeric(res.table.gc$lambda.bt), 3),"\u00B1", round(as.numeric(res.table.gc$stdlambda.bt),3)),
                                                   "A.boot" = paste(round(as.numeric(res.table.gc$A.bt), 3),"\u00B1", round(as.numeric(res.table.gc$stdA.bt),3)),
                                                   stringsAsFactors = F, check.names = F))
    }
  table_spline <- as.data.frame(apply(table_spline, 2, function(x) str_replace_all(x, "NA ± NA", "")))
  table_spline[is.na(table_spline)] <- ""
  knit_table(table_spline, caption = "Smooth Spline Fit")
}
```
\clearpage
## Plots
  ...
  for(i in 1:length(grofit[["gcFit"]][["gcFittedLinear"]])){
    if(grofit[["gcFit"]][["gcFittedLinear"]][[i]][["fitFlag"]] == TRUE){
        ...
        print(p + geom_segment(
          ...
          data = tangent.df,
          linetype = "dashed",
          color = ggplot2::alpha("firebrick3", 0.7),
          size = 0.5
        ))
    ...
    if(grofit[["gcFit"]][["gcFittedModels"]][[i]][["fitFlag"]] == TRUE){
         ...
         p <- p + annotate(
          ...
          annotate("text",
           ...
           scale_color_manual(name='Growth Model',
                       breaks = "logistic",
                       values=c("model" = ggplot2::alpha("forestgreen", 0.7), "logistic" = ggplot2::alpha("forestgreen", 0.7)))
      }
      if(x$model == "richards"){
        p <- p + annotate(
                "text",
          ...
          scale_color_manual(name='Growth Model',
                       breaks = "richards",
                       values=c("model" = ggplot2::alpha("forestgreen", 0.7), "richards" = ggplot2::alpha("forestgreen", 0.7)))
      }
      if(x$model == "gompertz"){
        p <- p + annotate(
              "text",
          ...
          scale_color_manual(name='Growth Model',
                       breaks = "gompertz",
                       values=c("model" = ggplot2::alpha("forestgreen", 0.7), "gompertz" = ggplot2::alpha("forestgreen", 0.7)))
      }
      if(x$model == "gompertz.exp"){
        ...
        p <- p + annotate(
          ...
          scale_color_manual(name='Growth Model',
                       breaks = "gompertz.exp",
                       values=c("model" = ggplot2::alpha("forestgreen", 0.7), "gompertz.exp" = ggplot2::alpha("forestgreen", 0.7)))
      ...
      p <- p + theme(legend.key = element_blank(),
       ...
       # /// add tangent at maximum slope
        ...
        print(p + geom_segment(aes(x = time[which.min(abs(bla))], y = y[which.min(abs(bla))],
                              ...
                              data = tangent.df, linetype = "dashed", color = ggplot2::alpha("forestgreen", 0.7), size = 0.5) +
          geom_segment(aes(x = time[1], y = y[1], xend = time[2], yend = y[2]), data = df.horizontal,
                       linetype = "dashed", color = ggplot2::alpha("forestgreen", 0.7), size = 0.5))
    }
  }
}
```
`r if(any(c("a", "b", "l", "m") %in% control$fit.opt)){"\\clearpage"}`
 ...
 for(i in 1:length(grofit[["gcFit"]][["gcFittedSplines"]])){
   if(grofit[["gcFit"]][["gcFittedSplines"]][[i]][["fitFlag"]] == TRUE){
       ...
       df <- data.frame("time" = x[["raw.time"]],
                        ...
                        "fit.data" = c(rep(NA, length(x[["raw.data"]])-length(x[["fit.data"]])), x[["fit.data"]]))
      ...
      p <-    ggplot(df, aes(x=time, y=data)) +
       ...
       scale_color_manual(name='Growth Model',
                       breaks = "Spline fit",
                       values=c("spline" = ggplot2::alpha("dodgerblue3", 0.7), "Spline fit" = ggplot2::alpha("dodgerblue3", 0.7)))
      
      # /// add tangent at maximum slope
       p.yrange.end <- ggplot_build(p)$layout$panel_params[[1]]$y.range[2]
       mu     <- as.numeric(coef$mu[1])
       time <- seq(lagtime, max(x$"fit.time"), length=200)
       bla <- coef["b.tangent"][[1]]+time*mu
       ...
       p <- p + geom_segment(aes(x = time[which.min(abs(bla))], y = y[which.min(abs(bla))],
                             xend = time[which.min(abs(y - p.yrange.end))],
                             yend = y[which.min(abs(y - p.yrange.end))]), 
                             data = tangent.df, linetype = "dashed", color = ggplot2::alpha("dodgerblue3", 0.7), size = 0.5) +
         geom_segment(aes(x = time[1], y = y[1], xend = time[2], yend = y[2]), data = df.horizontal,
                      linetype = "dashed", color = ggplot2::alpha("dodgerblue3", 0.7), size = 0.5)
...
```{r ec50_spline_plots, fig.width=7, fig.height=5, echo=F, warning=F, message=F, fig.align='center'}
plot.drFit(grofit$drFit)
```

```{r ec50.boot_summary, echo=F, warning=F, message=F, results='asis', eval = (is.numeric(grofit$control$nboot.dr) && grofit$control$nboot.dr > 0)}
  ...
  for (i in 1:length(grofit$drFit$drBootSplines)){
    boot.ec50.df <-
      plyr::rbind.fill(
        boot.ec50.df,
        data.frame(
          "Condition" = growth_fit$drFit$drTable$Test[i],
...
```{r ec50_boot_plots, fig.width=6, fig.height=4.5, echo=F, warning=F, message=F, fig.align='center'}
for(i in 1:length(grofit$drFit$drBootSplines)){
