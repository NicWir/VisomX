...
knitr::opts_knit$set(root.dir = str_replace_all(tempdir(), "\\\\", "/"))
...
-   PLS was performed on: `r if_else(mSetObj[["analSet"]][["plsda"]][["data.type"]]=="anova.sig", "significant features based on ANOVA\\.", "all features (including compounds that are non-significant according to ANOVA)\\.")`
\pagebreak
...
if(!is.null(mSetObj[["imgSet"]][["missval_heatmap"]])){
  print(mSetObj[["imgSet"]][["missval_heatmap"]])
}
```

```{r MissVal_Density_plot, fig.width=7, echo=FALSE, fig.align='center'}
if(!is.null(mSetObj[["imgSet"]][["missval_density"]])){
  gridExtra::grid.arrange(mSetObj[["imgSet"]][["missval_density"]])
}
```
...
tmp_list <- met.plot_FeatureNormSummary(mSetObj, imgName = paste0("Plots/FeatureNormSummary"), 
                                        plot = F, show_prenorm = T, export = T, format = "pdf")
```

```{r FeatureNormSummary_plot, echo=FALSE, fig.align='center'}
knitr::include_graphics(paste0(getwd(),"/Plots/FeatureNormSummary.pdf"),dpi = 300)
```
...
tmp_list <- met.plot_SampleNormSummary(mSetObj, imgName = paste0("Plots/SampleNormSummary"), 
                                       plot = F, show_prenorm = T, export = T, format = "pdf")
```

```{r SampleNormSummary_plot, echo=FALSE, fig.align='center'}
knitr::include_graphics(paste0(getwd(),"/Plots/SampleNormSummary.pdf"),dpi = 300)
```
...
dir.create("Plots/Volcano", showWarnings = F)
w = 9; h = w * 5.5/6.5
...
cat("A 3D PCA scores plot is available in the HTML version of this report.")
```
...
cat("A 3D PCA Loadings plot is available in the HTML version of this report.")
```
