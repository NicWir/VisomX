...
diff_true <- nrow(rowData(dep[rowData(dep, use.names = FALSE)$significant, 
    ])) != 0
knitr::opts_chunk$set(tidy.opts=list(width.cutoff=80), tidy=TRUE)
```
````{r,echo=FALSE}
contrasts <- rowData(dep) %>% 
...
## Summary
`r #browser()`
Original file contains _`r data@metadata$n.pre_filt`_ proteins, of which _`r data@metadata$n.filtered`_ were removed due to missing values (_filt_type = '`r data@metadata$filt_type`'_). 
...
if (length(contrasts) > 1){
  for (i in 1:length(contrasts)) {
    cat(paste0("* _",
               nrow(dep[rowData(dep)[[paste0(contrasts[i], "_significant")]],]),
...
* missing data was imputed using _`r colFmt(paste0("'", metadata(imp)$imp_fun, "'"),'blue')`_
* selected contrast type: `r colFmt(paste0("'", param[['type']], "'"),'blue')`
* tested contrasts:
  `r colFmt(gsub("_p.adj", "", colnames(rowData(dep))[grep("p.adj", colnames(rowData(dep)))]),'blue')`
...
#### Verify the variance stabilisation.\
  The purpose of the following graph is to determine if there is a systematic trend in the standard deviation of the data as a function of overall expression. These graphs are based on the assumption that most genes are not differentially expressed, so the running median is a reasonable estimate of the standard deviation of the data at the feature level, as a function of the mean. After vsn normalization, the running median should be approximately a horizontal line. There may be random fluctuations, but there should not be a general trend. If this is not the case, it usually indicates a data quality problem or is the result of inadequate data preprocessing.

SD-Rank(mean) plot before normalization:
```{=latex}
\begin{center}
```
```{r meanSDplot1,echo=FALSE, warning=FALSE, fig.width=5, fig.height=3}
...
\begin{center}
```
```{r meanSDplot2,echo=FALSE, warning=FALSE, fig.width=5, fig.height=3}
...
\begin{center}
```
```{r missval, echo=FALSE, fig.width = 7, fig.height = 6}
if (any(is.na(data.frame(assay(data))))) {
...
\begin{center}
```
```{r missval2, echo=FALSE, fig.width = 7, fig.height = 7}
if (any(is.na(data.frame(assay(data))))) {
...
\begin{center}
```
```{r density, echo=FALSE, fig.width = 7, fig.height = 9}
...
\begin{center}
```
```{r screeplot, echo=FALSE, fig.width = 7, fig.height= 6, warning=F, message=F}
ncomp <- 10
if(length(getComponents(pca)) < 10) ncomp = length(getComponents(pca))
withCallingHandlers(
    suppressWarnings(
        prot.plot_screeplot(pca, axisLabSize = 16, titleLabSize = 17, plot = T, export = F, 
                            components = getComponents(pca)[1:ncomp])
...
\begin{center}
```
```{r plot_pca_1_2, echo=FALSE, fig.width = 7, fig.height= 6, warning=F, message=F}
suppressMessages(
...
\begin{center}
```
```{r plot_pca_1_3, echo=FALSE, fig.width = 7, fig.height= 6, warning=F, message=F}
suppressMessages(
...
\pagebreak
### Loadings plot
PCA loadings are the coefficients of the linear combination of the original variables (proteins) from which the principal components (PCs) are generated. They describe how much each protein contributes to a particular principal component. Large loadings (positive or negative) indicate that a particular variable has a strong relationship to a particular principal component. The sign of a loading indicates whether a variable and a principal component are positively or negatively correlated.
```{=latex}
\begin{center}
```
```{r loadingsPlot, echo=FALSE, fig.width = 7, fig.height= 8, warning=F, message=F}
...
\begin{center}
```
```{r correlation_heatmap_all, echo=FALSE, fig.width = 7, fig.height= 7, warning=F, message=F, eval=diff_true}
    prot.plot_corrheatmap(dep, font_size = 12, significant = FALSE )
```
```{=latex}
\end{center}
```
...
\begin{center}
```
```{r correlation_heatmap, echo=FALSE, fig.width = 7, fig.height= 7, warning=F, message=F, eval=diff_true}
  prot.plot_corrheatmap(dep, font_size = 12, significant = T )
```
```{=latex}
\end{center}
```
...
len = nrow(dep[rowData(dep)$significant, ]) / 13
if (len < 5) { len = 5 }
```
```{=latex}
\begin{center}
```
```{r heatmap_1, echo=FALSE, fig.width = width*1.3, fig.height = len, warning=F, message=F}
try(suppressMessages(
  prot.plot_heatmap(
    dep,
    type = "contrast",
    kmeans = if_else(exists("heatmap.kmeans"), heatmap.kmeans, FALSE),
    k = if_else(exists("k"), k, NULL),
...
\pagebreak
### Volcano plot(s)
Volcano plots can be used to visualize a particular contrast (comparison between two samples). This allows to examine the protein enrichment between two samples (x-axis) and their corresponding (adjusted) p-values (y-axis).
```{=latex}
\begin{center}
```
```{r volcano, echo=FALSE, fig.width = 8, fig.height = 8.8, warning=F, message=F}
for (i in 1:length(contrasts)){
  prot.plot_volcano(dep, contrast = contrasts[i], 
                          add_names = T, label_size = 2.5, adjusted =  volcano.adjusted,
                          plot = T, export = F, lfc = metadata(dep)$lfc, alpha = metadata(dep)$alpha)
}
