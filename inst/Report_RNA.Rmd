...
```{r heatmap_contrast, echo=FALSE, warning=FALSE, message=FALSE}
# --- Dynamic figure dimensions ---
# width ~ samples
n_samp <- tryCatch(ncol(dds), error = function(e) 8L)
width  <- 5 + 0.45 * n_samp         # linear in samples
width  <- max(7, min(width, 14))    # clamp [7, 14] inches

# height ~ significant genes
rd <- SummarizedExperiment::rowData(dds, use.names = FALSE)
sig <- rd$significant; sig[is.na(sig)] <- FALSE
n_rows_sig <- sum(sig)

# ~0.06 inch per gene, clamp [4, 11] inches (works for HTML and PDF)
fig_h <- n_rows_sig * 0.06
fig_h <- max(4, min(fig_h, 11))

# share with subsequent chunks
assign(".__hm_width",  width,  envir = knitr::knit_global())
assign(".__hm_height", fig_h,  envir = knitr::knit_global())
```
```{r heatmap_1, echo=FALSE, fig.width = .__hm_width, fig.height = .__hm_height, warning=F, message=F, fig.align='center'}
if (isTRUE(export)) {
...
```
\pagebreak
```{r heatmap_condition, echo=FALSE, fig.width = .__hm_width, fig.height = .__hm_height, fig.align='center'}
if (isTRUE(export)) {
